...
namespace AnkouBlog.Controllers
{
    ...
    public class TopicDetailController : ControllerBase
    {
        ...
        public async Task<MessageModel<PageModel<TopicDetail>>> Get(int page = 1, string tname = "", string key = "")
        {
            ...
            var data = await _topicDetailServices.QueryPage(a => !a.tdIsDelete && 
            a.tdSectendDetail == "tbug" && ((tid == 0 && true) || (tid > 0 && a.TopicId == tid)) && 
            ((a.tdName != null && a.tdName.Contains(key)) || (a.tdDetail != null && a.tdDetail.Contains(key))), 
