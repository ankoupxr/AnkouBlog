...
namespace AnkouBlog.Controllers
{
    ...
    public class PermissionController : ControllerBase
    {
        ...
        public async Task<MessageModel<string>> Assign([FromBody] AssignView assignView)
        {
            ...
            try
            {
                if (assignView.rid > 0)
                {
                    ...
                    var roleModulePermissions = await _roleModulePermissionServices.Query(d => d.RoleId ==
                    assignView.rid);
        ...
        public async Task<MessageModel<NavigationBar>> GetNavigationBar(int uid)
        {
            ...
            var uidInHttpcontext = (JwtHelper.SerializeJwt(_httpContext.HttpContext.Request.Headers["Authorization"].
                ObjToString().Replace("Bearer ", "")))?.Uid;

            if (uid > 0 && uid == uidInHttpcontext)
            {
                var roleId = ((await _userRoleServices.Query(d => d.IsDeleted == false &&
                d.UserId == uid)).FirstOrDefault()?.RoleId).ObjToInt();
                if (roleId > 0)
                {
                    var pids = (await _roleModulePermissionServices.Query(d => d.IsDeleted == false && 
                    d.RoleId == roleId)).Select(d => d.PermissionId.ObjToInt()).Distinct();

                    if (pids.Any())
                    {
                        var rolePermissionMoudles = (await _permissionServices.Query(d => pids.Contains(d.Id) &&
                        d.IsButton == false)).OrderBy(c => c.OrderSort);
